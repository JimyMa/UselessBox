...
std::pair<GF256_poly, GF256_poly> Berlekamp_Decoder::GetSigmaOmega(
  ...
  for (unsigned int i = 1; i <= 2 * get_max_correct(); i++) {
    ...
    if (delta == 0 || (2 * D) > i || (delta != 0 && (2 * D) == i && B == 0)) {
      D = D;
      B = B;
    ...
    else if (delta != 0 && ((2 * D) < i || ((2 * D == i) && B == 1))) {
      D = i - D;
      B = 1 - B;
      tau = sigma / delta;
      gamma = omega / delta;
...
GF256_poly Berlekamp_Decoder::CheinSearch(GF256_poly& sigma_poly,
  ...
  for (int i = 0; i < loc.size(); i++) {
    ...
    dec_poly = dec_poly + GF256_poly(loc[i], err_value);
  }
  return dec_poly;
}
